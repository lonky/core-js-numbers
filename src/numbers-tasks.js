...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const xSide = absX1 + absX2;
  const ySide = absY1 + absY2;
  const res = Math.sqrt(xSide ** 2 + ySide ** 2);
  return res;
}
...
function roundToPowerOfTen(num, pow) {
  const afterPow = num + (num % 10 ** pow);
  const res = afterPow - (afterPow % 10 ** pow);
  return res;
}
...
function isPowerOfTwo(num) {
  let res = false;
  if (2 ** Math.floor(Math.log2(num)) === num) {
    res = true;
  }
  return res;
}
